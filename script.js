...
const formatDate = (date, locale) => {
  const calcDaysPassed = date =>
    Math.round(Math.abs(date - new Date()) / (24 * 60 * 60 * 1000));
  ...
  const days = calcDaysPassed(date);
  ...
  movs.forEach(function (mov, index) {
    ...
    const html = `
      ...
      index + 1
        ...
        <div class="movements__date">${formatDate(now, acc.locale)}</div>
        <div class="movements__value">${formatValue(
          mov,
          acc.locale,
          acc.currency
        )}</div>
...
const displayBalance = function (account) {
  ...
  labelBalance.innerHTML = formatValue(
    balance,
    account.locale,
    account.currency
  );
...
const displaySummary = function (account) {
  //Calculate total Desposib
  const deposit = account.movements
    .filter(mov => mov > 0)
    .reduce((acc, mov) => acc + mov, 0);

  labelSumIn.textContent = formatValue(
    +deposit.toFixed(2),
    account.locale,
    account.currency
  );
  ...
  labelSumOut.textContent = formatValue(
